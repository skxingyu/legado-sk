...
- ✅ **10033（`3.26.090613c`）已装平板 HA1KAPWG（2026-09-06）——当前交付（待真机回归楼中楼展开确认后转正发布）**：修复「评论快照楼中楼（'N条回复'）在 10032 强展后仍点不开」的真正根因。10032 的 `60910f91` 强展用纯文本正则命中 toggle，expandRe 漏配"N 条回复"（按钮实为 `<span>1 条回复</span>`），导致从未真正点开任何 reply-toggle（实拉快照统计 19 个 `.reply-toggle` 全 `display:none` 收起、0 展开）。**`d26f2798` 修复**：`FORCE_EXPAND_REPLIES_JS` 改两段式——① 结构定位契约类 `.reply-toggle` 直接把它后兄弟 `.replies-container` 强制显示（回复 DOM 本就预置、非懒加载，纯 DOM 操作零往返）；② 保留文本兜底并补 `\d+条/个/楼回复` 正则 + "跳过祖先已标记"防重入。jsdom 在真实快照 HTML 验证 round1 展开/round2 收口 PASS。产物 `release/legado_sk_3.26.090613c_10033_arm64-v8a.apk`（27,397,149 字节），aapt(包名/10033/阅读SK/arm64-v8a) + apksigner 通过，平板覆盖安装冷启动无 FATAL。
- 10031（`3.26.090604c`）/10032（`3.26.090605c`）为平板实装中间版，已被 10033 取代：10031 = ②朗读变速步长折算（`a510d48c`）+ ③刷新真正下载章评（`49e962f2`）实施版；10032 = ①并发可配默认 4（`8050c7e9`）+ ②楼中楼强展初版（`60910f91`）。均未独立发布，净功能已并入 10033。
- 相较 10030（`8e23ead`）累积净增量：② TTS 变速句内过界步长折算（`a510d48c`）、③ 刷新当前章触发 REVIEW 下载 + 显式重复下载重抓最新（`49e962f2`）、① 快照抓取并发可配默认 4（`8050c7e9`）、② 快照楼中楼强展真修（`60910f91`→`d26f2798`）。
- **回退边界**：`d26f2798` 之前 = 10032（`60910f91`）；`60910f91` 之前 = 10031；`49e962f2` 之前 = 10030（`8e23ead`）。10030 及更早（含 10026/27/29 等 Pre）发布信息见历史。
- 下一次交付 versionCode 从 `10034` 递增。
