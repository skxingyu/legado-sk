...
- Pre 版：`3.26.082414c` / `10020`，2026-08-24 发布（tag `v3.26.082414-10020`，prerelease，提交 `c75e669`+`154d84d`）。内容：MD3-main 格式主题包导入（主题管理 → 添加 → 导入 ZIP，日/夜拆双包落地）+ 管理样式面板「导入」按钮显示修复（无头 Alert 复合 customTitle 整体迁移）。两项均经用户实机验证。模拟器已装本版。
