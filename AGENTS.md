...
- Pre 版：`3.26.082414c` / `10020`，2026-08-24 发布（tag `v3.26.082414-10020`，prerelease，提交 `c75e669`+`154d84d`）。内容：MD3-main 格式主题包导入（主题管理 → 添加 → 导入 ZIP，日/夜拆双包落地）+ 管理样式面板「导入」按钮显示修复（无头 Alert 复合 customTitle 整体迁移）。两项均经用户实机验证。
- 本地构建（未发布）：`3.26.082417c` / `10021`，2026-08-24，提交 `c852d43`+`f118ea9`+`3d36b60`。内容：移植 Legado_Max 三项修复——漫画章末图片不再撑高至屏幕 2/3、目录加载失败保留旧目录、书源地址变更自动迁移关联书籍。模拟器已装本版；用户要求暂不发 GitHub Release，待实机验证后再定。后续发布时以本 versionCode 为准（不得复用）。
