...
7. ⚠️ 三份 companion 文档在 `.gitignore` 中忽略、不推送公开仓库；只读不随意改动，改动需与对应事实一致。若发现文档与源码事实不符，以源码和 GitHub Releases 为准并先核实再改文档。
...
- 正式版 = `app` flavor + `release` buildType（`assembleAppRelease`，包名 `io.legado.app.c`）。`versionName` 需直接传完整值（含 `c` 后缀，如 `3.26.090812c`），无自动加后缀机制；`versionCode` 遵循 SK 独立递增约定（当前基线 10037）。
