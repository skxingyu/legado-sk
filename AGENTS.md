...
- 🚧 **10023 已发布 Pre（2026-08-29）**（`v3.26.082907-10023`，提交 `a824a0f`+`ce5fb53`）：**整批移植上游 legadoC 8/26-8/27 朗读架构收束**——两原语（`setAloudStart` 双击换段 / `backToAloudProgress` 回原进度）+ 纯函数跟随规则 `shouldFollowAloudAdvance` + 派生脱节 `isViewBehindAloud`，替换 10022 的存储式方案；`readAloudByPage` 拆为 `pageSplit`（页间分段）+ `forcePageFollow`（强制追页），新增页首按段；朗读红字改绘制期投影；系统 TTS 预测换页 + HTTP 被动预测；位置事件 `READ_ALOUD_POSITION`（带 generation 防乱序）取代 `TTS_PROGRESS`。DB 仍为 105。10021 仍为 Latest，10023 待完整验证后转正。
- 🚧 **10024 已编译待真机测试（2026-08-29，`3.26.082912c`，提交 `8fd65f0`+`d2c3924`+`bc78a13`，未推送）**：代码审查修复批次 A（数据安全）+ B（朗读回归，翻页动画守卫/滚动模式拽页首/双击换段/落库）+ **听书底部悬浮窗「先贴屏底后回正」修复**（`bc78a13`：定位与显示解耦，`tryReadAloudPanelBottomMargin` + `positionAndRevealReadAloudPanel` + PageView/ReadView `footerMeasurable`，悬浮窗页脚未就绪时保持隐藏，绝不先以贴底位置闪现）。产物 `release/legado_sk_3.26.082912c_10024_arm64-v8a.apk`（33,761,488 字节，已覆盖根目录「当前交付 APK」）。**待用户真机测试（听书悬浮窗 + 朗读五要点 + 进度同步）后发布**。
- 🚧 **10025 已发布 Pre（2026-08-31，`3.26.0831c`，提交 `cef9d84`+`7a4ebae`+`19bc2dd`，已推送 main）**：基于全项目代码审查的高性价比批次（M1-M4 + N1 + L2）+ **切书闪退修复**。① 朗读边界加固：`BaseReadAloudService` 避让 bounds 非法/无候选不再崩溃（`check`/`error` 改日志+忽略/回退）、`ReadBook.resetData` 切书清 `aloudPosition`（消除跨书串扰）、`prevP` 补 `pageIndex > 0` 下界、`prevP`/`nextP` 的 `paragraphs[nowSpeak]` 改 `getOrNull` 防越界。② 进度同步数据安全：`AppWebDav.getBookProgress` 对云端文件存在但内容非合法 JSON 抛异常中止上传（不再当作首同步覆盖）；`migration_103_104` 的 DROP INDEX 补 `IF EXISTS`。③ **切书闪退修复（`19bc2dd`，用户实测确认）**：`positionAndRevealReadAloudPanel` 原手写 `viewTreeObserver.addOnGlobalLayoutListener` 捕获过期 ViewTreeObserver 引用，切书/Activity 重建 View 树后回调 `removeOnGlobalLayoutListener` 抛 `IllegalStateException` 闪退；改用 KTX `doOnLayout`（动态取当前 observer + isAlive 守卫）+ `isAttachedToWindow` 守卫。产物 `release/legado_sk_3.26.0831c_10025_arm64-v8a.apk`（32.20MB，已覆盖根目录「当前交付 APK」）。**10021 仍为 Latest**，10025 为 Pre，待完整验证后转正。
- 下一次交付 versionCode 从 `10026` 递增（10025 Pre 完整验证后按 10021 方式转正）。
