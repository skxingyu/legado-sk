...
- 正式版（Latest）：`3.26.082310c` / `10019`，2026-08-23 发布（tag `v3.26.082310-10019`）。内容：「接收预发布版本」更新开关（`updateCheckPre`，默认关；用户实机验证通过后由 pre 转正）。
- Pre 版：`3.26.082414c` / `10020`，2026-08-24 发布（tag `v3.26.082414-10020`，prerelease）。内容：MD3-main 格式主题包导入 + 管理样式面板「导入」按钮显示修复。**该 pre 将被下方待发正式版替换**。
- 待发正式版（用户测试中）：`3.26.082507c` / `10020`，2026-08-25 同号覆盖重编，提交 `c852d43`+`f118ea9`+`3d36b60`+`99669ee`。在 pre 内容之上新增四项：移植 Legado_Max 三项修复（漫画章末图片不再撑高至屏幕 2/3、目录加载失败保留旧目录、书源地址变更自动迁移关联书籍）+ 听书呼出菜单改为普通主菜单（长按「朗读」才进听书专属面板，悬浮窗不变）。模拟器已装本版；用户要求测试通过后再发布：届时删除旧 `v3.26.082414-10020` 的 release 与 tag，以本产物重建正式版 `--latest`，避免应用内更新检测遇到重复 versionCode。同号中间产物 `legado_sk_3.26.082417c_10020_arm64-v8a.apk` 已被本版取代（仅存于 release 目录存档）。
- 曾编过 `3.26.082417c` / `10021` 本地包（同三项移植内容），因改定 10020 为发布形态而废弃，从未对外发布。
- 下一次交付 versionCode 从 `10021` 递增。
