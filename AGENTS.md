...
- 正式版（Latest）：`3.26.082507c` / `10020`，2026-08-25 发布（tag `v3.26.082507-10020`，提交 `d6da042`）。内容：MD3-main 格式主题包导入 + 管理样式面板「导入」按钮显示修复 + 移植 Legado_Max 三项修复（漫画章末图不撑高/目录加载失败保留旧目录/书源地址变更自动迁移关联书籍）+ 听书呼出菜单改普通主菜单（长按「朗读」进专属面板）。全部经用户实机验证；旧 pre `v3.26.082414-10020` 的 release 与 tag 已删除。
- 本地构建（未发布）：`3.26.082507c` / `10021`，2026-08-25，提交 `a0d968f`。内容：修复同号 pre 转正后无法触发内置更新的缺陷——更新候选排序改为三级键（versionCode → 正式版优先 → versionName 编译时刻 MMddHH），更新判定在 versionCode 相同时追加「线上编译时刻比本地新即提示」，使已装同号 pre 版的设备能收到转正正式版推送；pre 用户未开 updateCheckPre 开关也能收到（正式版不被 prerelease 过滤）。模拟器已装本版待用户测试。
