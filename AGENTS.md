...
- ✅ **10035（`3.26.090801c`）已装平板 HA1KAPWG（2026-09-08）——当前交付（实机回归通过）**：**朗读引擎选择新增「网络导入」**（commit `0bb36aac`）：在朗读引擎选择弹窗（`SpeakEngineDialog`）工具栏新增 `menu_import_net` 菜单项（复用 `import_on_line`「网络导入」文案），点击弹 URL 输入框，用 OkHttp 拉取引擎 JSON（支持 `#requestWithoutUA` 尾缀），再按内容自动分流——脚本引擎(V2 json)走 `TtsEngineStore.importEngineText`（含冲突覆盖/保留副本确认），HTTP 引擎走复用 `ImportHttpTtsDialog` 选择导入界面。本地导入/直接填写入口不受影响。产物 `release/legado_sk_3.26.090801c_10035_arm64-v8a.apk`（27,398,495 字节），aapt(包名 10035/3.26.090801c/阅读SK/arm64-v8a) + apksigner 通过，平板覆盖安装后实机回归「网络导入」菜单项与分流导入可用通过。
- 10034（`3.26.090706c`，2026-09-07，已装平板实机回归进书提速通过）为前一交付，净增量见历史：进书加载提前到布局完成执行（`03f0f805`/legadoC `9c23b7e7`）。10034 未独立发布 Release。
- 相较 10030（`8e23ead`）累积净增量：② TTS 变速句内过界步长折算（`a510d48c`）、③ 刷新当前章触发 REVIEW 下载 + 显式重复下载重抓最新（`49e962f2`）、① 快照抓取并发可配默认 4（`8050c7e9`）、② 快照楼中楼强展真修（`60910f91`→`d26f2798`）、④ 进书加载提前到布局完成执行（`03f0f805`/legadoC `9c23b7e7`）、⑤ 朗读引擎选择网络导入（`0bb36aac`）。
- **回退边界**：`0bb36aac`（10035，网络导入）之前 = 10034（`03f0f805`）；`03f0f805`（10034，进书加载前置）之前 = 10033（`d26f2798`）；`d26f2798` 之前 = 10032（`60910f91`）；`60910f91` 之前 = 10031；`49e962f2` 之前 = 10030（`8e23ead`）。10030 及更早（含 10026/27/29 等 Pre）发布信息见历史。
- 下一次交付 versionCode 从 `10036` 递增。
