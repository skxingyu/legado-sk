...
- 待发正式版（用户测试中）：`3.26.082417c` / `10020`，2026-08-24 同内容重编，提交 `c852d43`+`f118ea9`+`3d36b60`+`02f6700`。在 10020 pre 内容之上新增移植 Legado_Max 三项修复——漫画章末图片不再撑高至屏幕 2/3、目录加载失败保留旧目录、书源地址变更自动迁移关联书籍。模拟器已装本版（自 10021 降级）；用户要求测试通过后再发布：届时删除旧 `v3.26.082414-10020` 的 release 与 tag，以本产物重建正式版 `--latest`，避免应用内更新检测遇到重复 versionCode。
