 ...
 * 这里锁定的三个事实一旦被破坏，守卫会**静默失效**（书源照常工作、非授权客户端也能用）：
 * 1. 规则仍以 `@js:` 开头——legado 只识别首位的 `@js:`，被挤到第二行整段会退化为字面量；
 * 2. 承载结果的入口都调用了 `fqAuthOk`；
 * 3. `jsLib` 声明的包名/应用名与 SK 版实际值一致，且提示文案里的换行已正确转义。
 */
class BuiltinSourceGuardTest {
    ...
    @Test
    fun everyResultEntryRuleCallsGuard() {
        // 守卫落在真正承载结果的入口：搜索/发现产出书单，正文产出章节内容。
        // searchUrl 只负责拼 URL，不产出结果，故不参与本断言。
        listOf(
            "ruleSearch.bookList",
            "ruleExplore.bookList",
            "ruleContent.content",
        ).forEach { path ->
            assertTrue("$path 未调用 fqAuthOk", rule(path).contains("fqAuthOk"))
        }
    }
