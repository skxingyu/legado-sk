...
object ReadBook : CoroutineScope by MainScope() {
    ...
    ): TextChapter? = withContext(IO) {
        val book = book ?: return@withContext null
        val cachedChapter = when (index) {
            prevTextChapter?.chapter?.index -> prevTextChapter
            curTextChapter?.chapter?.index -> curTextChapter
            nextTextChapter?.chapter?.index -> nextTextChapter
            else -> null
        }
        if (cachedChapter?.isCompleted == true) {
